/* app.js — Transforma-Ação Climática (INMET)
   VERSÃO FINAL ESTÁVEL — multi-anos via assets/years.json
*/
...
function byUFName(a,b){
  return (`${a.uf}${a.name}`).localeCompare(
    `${b.uf}${b.name}`,
    "pt-BR",
    { sensitivity:"base" }
  );
...
function renderList(){
  ...
  for(const st of filtered){
    ...
    div.innerHTML = `
      <div class="name">${safe(st.name)}
        <span style="color:#64748b;font-weight:800">(${st.uf})</span>
      </div>
      <div class="meta">
        ID ${st.id} • ${niceCoord(st.lat)}, ${niceCoord(st.lon)}
      </div>
    `;
...
function applyFilter(){
  ...
  filtered = q
    ? STATIONS.filter(st =>
        `${st.id} ${st.name} ${st.uf}`.toLowerCase().includes(q)
      )
    : STATIONS.slice();
...
async function loadAndPlot(stationId, year){
  ...
  try{
    ...
    el("chart").innerHTML = `
      <div style="padding:14px;color:#b91c1c;font-weight:700">
...
function renderCards(d){
  const a = d.annual || {};
  el("cards").innerHTML = [
    ["T mín (ano)",`${fmt(a.tmin)} °C`],
    ["T méd (ano)",`${fmt(a.tmean)} °C`],
    ["T máx (ano)",`${fmt(a.tmax)} °C`],
...
function renderInsights(d){
  ...
  if(!box || !d.months?.length){
    box.innerHTML = "";
    return;
  }

  const maxP = d.months.reduce((a,b)=> b.p>a.p?b:a);
  const minP = d.months.reduce((a,b)=> b.p<a.p?b:a);

  box.innerHTML = `
    <div class="insTitle">Resumo climático</div>
    <div>📍 ${d.station} • ${d.year}</div>
    <div>🌧️ Mês mais chuvoso: ${MONTHS[maxP.m-1]} (${fmt(maxP.p)} mm)</div>
    <div>🏜️ Mês mais seco: ${MONTHS[minP.m-1]} (${fmt(minP.p)} mm)</div>
  `;
}
...
function plotClimogram(d){
  ...
  Plotly.newPlot("chart",[
    ...
    { x, y:t, type:"scatter", yaxis:"y2", name:"Temp. média (°C)" }
  ],{
...
async function bootstrap(){
  ...
  el("year").addEventListener("change", ()=>{
    if(selectedStation){
      loadAndPlot(selectedStation.id, el("year").value);
    }
  });
  ...
  el("stationMeta").textContent = "—";
  el("chart").innerHTML = `
    <div style="padding:14px;color:#475569">
